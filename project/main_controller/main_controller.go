...
func Main_controller(controller_id int,
	...
	controller_state_machine(state, controller_id, elev_to_ctrl_chan, elev_to_ctrl_button_chan, ctrl_to_elev_chan, ctrl_to_elev_cab_chan, network_receive_order_chan, network_send_chan)
...
func controller_state_machine(state State,
	...
	ctrl_to_elev_chan chan<- [utilities.N_FLOORS][utilities.N_BUTTONS-1]bool,
	ctrl_to_elev_cab_chan chan<- elevio.ButtonEvent,
	network_receive_chan <-chan utilities.OrderDistributionMessage,
	network_send_chan chan<- utilities.StatusMessage) {

	switch state {
	case state_normal:
		fmt.Println("Starting normal controller")
		controller_modes.Normal_controller(controller_id, elev_to_ctrl_chan, elev_to_ctrl_button_chan, ctrl_to_elev_chan, ctrl_to_elev_cab_chan, network_receive_chan, network_send_chan)
	case state_backup:
